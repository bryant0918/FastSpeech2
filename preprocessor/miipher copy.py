...
class MiipherInference:
    ...
    def process_directory(self, speakers):
        ...
        # Set up processing speaker function for multithreading purposes
        def processing_speaker(file_name):
        ...
        for i, speaker in enumerate(tqdm(os.listdir(self.input_dir))):
            ...
            file_list = os.listdir(os.path.join(self.input_dir, speaker))
            ...
            with ThreadPoolExecutor(max_workers=21) as executor:
                list(tqdm(executor.map(processing_speaker, file_list), total=len(file_list)))
